...
<body>
	...
	<!-- 리뷰 목록 -->
	<div>
	<form action="<%=request.getContextPath()%>/updateReviewForm.jsp?reviewNo=<%=re.get("reviewNo")%>&goodsNo=<%=goodsNo%>" method="post">

	<input type="hidden" name=updateDate value="<%=re.get("updateDate")%>">
	<input type="hidden" name=reviewContent value="<%=re.get("reviewContent")%>">
	<table style=" margin-left:auto; margin-right:auto; " class="table table-bordered" >
			<thead>
				<tr>
					<td>리뷰번호</td>
			...
			<tr>
				...
				<td><button type="submit" class="btn btn-dark">리뷰수정</button></td>
